   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#FE8010","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.goodsCollectionValue}}</view>
					</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('goodsCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.goodsCollectionPhoto ? this.detail.goodsCollectionPhoto.split(",") : [];
			},
